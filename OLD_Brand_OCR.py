...
def identify_unknown_brand_using_VLM(image_path):
    ...
    prompt = """
        ...
        1. Identify the brand name (if applicable).
        2. Identify the parent company (if applicable).
        3. Classify the advertisement into EXACTLY ONE category from the list below.

        CATEGORY LIST (choose ONLY one, do NOT invent new categories):
        {CATEGORIES}
    ...
    url = 'http://4c-desktop:11434/api/generate' # Changed to /generate for simpler output
...
#     # 🔥 CONTROLLED PROMPT
#     prompt = f"""
#     You are analyzing a newspaper advertisement image.

#     Tasks:
#     1. Identify the brand name (if visible).
#     2. Identify the parent company (if applicable).
#     3. Classify the advertisement into EXACTLY ONE category from the list below.
...
#     Rules:
#     - If brand not found → "Unknown"
#     - If parent company not found → "Unknown"
...
#     url = 'http://4c-desktop:11434/api/generate'
...
#     # "options": {
#     #         "temperature": 0   # 🔥 IMPORTANT: makes output more consistent
#     #     }
